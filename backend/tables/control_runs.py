...
def merge_control_run_data(run_data):
    ...
    control_run_id_str = str(uuid.uuid4())

    # Retrieve the schema from the Delta table
    table_schema = spark.read.format("delta").load(table_paths['control_runs']).schema
    ...
    control_run_row = Row(
        department_id=run_data[0],
        feature_id=run_data[1],
        feature_version=run_data[2],
        feature_data=run_data[3],  # Assuming this is already in the correct format
        control_id=run_data[4],
        control_version=run_data[5],
        control_signal_count=run_data[6],
        control_signal_data=run_data[7],  # Assuming this is already in the correct format
        signal_detected=run_data[8],
        control_run_id=control_run_id_str,
    )

    # Convert list of data to DataFrame
    df = spark.createDataFrame([control_run_row], schema=table_schema)
